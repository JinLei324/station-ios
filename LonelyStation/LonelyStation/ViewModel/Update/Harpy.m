...
+ (void)checkVersion{
    [self checkVersion:NO andView:nil];
}
        ...
        if ( [data length] > 0 && !error ) { // Success
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if ( ![versionsInAppStore count] ) { // No versions of app in AppStore
                    [Harpy sharedInstance].shouldUpdateVersion = NO;
                    return;
                    ...
                    if ([kHarpyCurrentVersion compare:currentAppStoreVersion options:NSNumericSearch] == NSOrderedAscending) {
                        [Harpy sharedInstance].shouldUpdateVersion = YES;
                        if (sholdalert) {
...
+ (void)showAlertWithAppStoreVersion:(NSString *)currentAppStoreVersion
{
    ...
    if ( harpyForceUpdate ) { // Force user to update app
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:kHarpyAlertViewTitle
                                                            message:[NSString stringWithFormat:@"%@ %@ %@.", appName,kHarpyAlertViewMSG, currentAppStoreVersion]
                                                           delegate:self
