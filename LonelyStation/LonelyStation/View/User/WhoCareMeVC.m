...
@interface WhoCareMeVC()<UITableViewDelegate,UITableViewDataSource,SWTableViewCellDelegate>{
...
@implementation WhoCareMeVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self.viewNaviBar setTitle:Local(@"WhoCareMe") andColor:RGB(145,90,173)];
    [self initView];
...
-(void)loadData:(BOOL)showHUD{
    ...
    [_mainViewVM getWhoCareMeWithBlock:^(NSArray<LonelyStationUser *> *arr, BOOL ret) {
        //这里没有做分页
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell hideOther];
    DLog(@"addTIme==%@",obj.addTime);
    cell.lastOnlineLabel.text = [EMUtil getTimeToNow:obj.addTime];
