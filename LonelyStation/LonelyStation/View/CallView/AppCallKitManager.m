...
{
    NSLog(@"resetedUUID:%d",(int)provider.pendingTransactions.count);
}
...
{
    NSLog(@"CallKit--callChanged--callObserver:::%d----call.isOnHold---:::%d--call.isOutgoing--:::%d--call.hasConnected--:::%d---call.hasEnded--:::%d",(int)callObserver.calls.count,call.isOnHold,call.isOutgoing,call.hasConnected,call.hasEnded);
...
- (CXProviderConfiguration *)configuration
{
    if (!_configuration) {
        ...
        _configuration = [[CXProviderConfiguration alloc] initWithLocalizedName:appName];//一般是app名字
        ...
        _configuration.iconTemplateImageData = [NSData dataWithData:UIImagePNGRepresentation(image)];//锁屏图标 40*40 px
//        NSString *ringPath = [[NSBundle mainBundle] pathForResource:@"ringtone" ofType:@"mp3"];
        _configuration.ringtoneSound = @"ringtone.mp3"; //这是个亮点 打电话的app可以做到自定义来电铃声
