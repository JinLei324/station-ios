...
@interface ChatVIPVC ()<UITableViewDelegate,UITableViewDataSource>{
...
- (void)viewDidLoad {
    ...
    [self.viewNaviBar setTitle:Local(@"ChatCard")];
    _mainViewVM = [[UserSettingViewModel alloc] init];
    selectIndex = -1;
    [self initViews];
    [UIUtil showHUD:self.view];
    [_mainViewVM getChatProgramWithBlock:^(NSArray *array, BOOL ret) {
