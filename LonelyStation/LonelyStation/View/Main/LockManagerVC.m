...
@interface LockManagerVC()<UITableViewDelegate,UITableViewDataSource,SWTableViewCellDelegate>{
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.viewNaviBar setTitle:Local(@"LockSetting") andColor:RGB(145,90,173)];
    [self initView];
...
-(void)loadData:(BOOL)showHUD{
    ...
    [_mainViewVM getMyLockListWithStart:[NSString stringWithFormat:@"%d",_from] andNumbers:[NSString stringWithFormat:@"%d",_cnt] andBlock:^(NSArray<LonelyStationUser *> *arr, BOOL ret) {
        //这里没有做分页
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.durationLabel.text = Local(@"SureLock");
    cell.durationLabel.textAlignment = NSTextAlignmentLeft;

    cell.lastOnlineLabel.text = [EMUtil getTimeToNow:obj.blockByMeTime];
...
-(NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewRowAction *button = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:Local(@"CancelLock") handler:^(UITableViewRowAction *action, NSIndexPath *indexPath)
                                    {
                                        LonelyStationUser *lonelyUser = _dataArray[indexPath.row];
                                        //取消封锁
                                        AllPopView *alert = [[AllPopView alloc] initWithTitle:Local(@"Warning") message:Local(@"SureCancelLock") clickedBlock:^(AllPopView *alertView, BOOL cancelled, NSInteger buttonIndex) {
                                            if (!cancelled) {
                                                [_mainViewVM deleteLockWithId:lonelyUser.userID andBlock:^(NSDictionary *dict, BOOL ret) {
                                                    if (dict && ret) {
