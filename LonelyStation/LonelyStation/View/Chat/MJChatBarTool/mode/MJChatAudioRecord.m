...
- (BOOL)finshishRecord
{
    ...
    audioRecordModel.duartion = _duration;
   return YES;
}
- (void)cancleRecord
{
...
- (BOOL)createRecord
{
    ...
    /* 创建输入音量更新 */
    soundMouterTimer = [NSTimer scheduledTimerWithTimeInterval:0.05 target:self selector:@selector(updateSoundMouter:) userInfo:nil repeats:YES];
...
- (void)updateSoundMouter:(NSTimer *)timer
{
    ...
    float peakPower = [audioRecord averagePowerForChannel:0];
    double ALPHA = 0.05;
    double peakPowerForChannel = pow(10, (ALPHA * peakPower));
    _soundMouter = peakPowerForChannel;
    if (_mouterDelegate) {
        [_mouterDelegate didUpDateMouter:_soundMouter];
    }
}
