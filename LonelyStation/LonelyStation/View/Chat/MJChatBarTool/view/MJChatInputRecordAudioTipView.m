...
- (void)setupStyle
{
    ...
    self.minRecordTimeErrorTitle = Local(@"SpeakToShort");
    self.maxRecordTimeErrorTitle = Local(@"SpeakToLong");
    self.upToCancelRecordTitle = Local(@"UpCancelRecord");
    self.releaseToCancelRecordTitle = Local(@"ReleaseCancelRecord");
    ...
    if (!self.willCancel) {
        if (self.voiceMicImage) {
            ...
            if (self.soundMeter >= 0) {
                if (self.soundMeter > 0.8) {
                    self.voiceMicImage = [UIImage imageNamed:@"chat_6"];
                }else if(self.soundMeter > 0.6){
                    self.voiceMicImage = [UIImage imageNamed:@"chat_5"];
                }else if(self.soundMeter > 0.4){
                    self.voiceMicImage = [UIImage imageNamed:@"chat_4"];
                }else if(self.soundMeter > 0.3){
                    self.voiceMicImage = [UIImage imageNamed:@"chat_3"];
                }else if(self.soundMeter > 0.2){
                    self.voiceMicImage = [UIImage imageNamed:@"chat_2"];
                }else if(self.soundMeter > 0.1){
                    self.voiceMicImage = [UIImage imageNamed:@"chat_1"];
                }else{
                    self.voiceMicImage = [UIImage imageNamed:@"chat_0"];
                }
                CGSize micSize = self.voiceMicImage.size;
                
                CGFloat topMargin = 30;
                
                micRect = (CGRect){(self.bounds.size.width-micSize.width)*0.5,topMargin,micSize.width,micSize.height};
